...
func toMountPoints(values []string) ([]*ecs.MountPoint, error) {

	points := []*ecs.MountPoint{}

	for _, value := range values {
		mp, err := toMountPoint(&value)

		if err != nil {
			return []*ecs.MountPoint{}, err
		}
...
func toMountPoint(value *string) (*ecs.MountPoint, error) {
